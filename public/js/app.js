...
const App = (() => {
        ...
        if (rulings.length > 0) {
          decisions = rulings.map(r => {
            ...
            return {
              ...
              amount: (ruling.paymentAmount || r.paymentAmount || escrows.find(e => e.escrowId == r.escrowId)?.paymentAmount || 0) / 1e6,
              reasoning: ruling.reasoning || (ruling.votes && ruling.votes[0]?.reasoning) || ruling.analysis || r.reasoning || r.analysis || '',
              confidence: ruling.confidence || (ruling.votes && ruling.votes[0]?.confidence) || r.confidence || null,
              model: ruling.model || (ruling.votes && ruling.votes[0]?.model) || r.model || 'Grok 4.1',
  ...
  async function downloadBlob(url, filename, contentType) {
    try {
      ...
      // Fallback for mobile browsers that ignore a.click download
      setTimeout(() => {
        if (document.hasFocus()) window.open(blobUrl, '_blank');
