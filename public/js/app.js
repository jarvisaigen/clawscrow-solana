    ...
    el.innerHTML = filtered.map(d => `
      <div class="decision-row" style="cursor:pointer" onclick="this.querySelector('.decision-details').classList.toggle('hidden')">
        ...
        <div class="decision-details hidden" onclick="event.stopPropagation()">
          ${d.buyerArgument ? `<div style="margin:0.5rem 0;padding:0.5rem;background:rgba(255,100,100,0.1);border-radius:6px"><strong>🗣️ Buyer's Argument:</strong><br>${d.buyerArgument}</div>` : ''}
          ${d.sellerArgument ? `<div style="margin:0.5rem 0;padding:0.5rem;background:rgba(100,255,100,0.1);border-radius:6px"><strong>🗣️ Seller's Argument:</strong><br>${d.sellerArgument}</div>` : ''}
